...
namespace CDCE.Dumper
{
    class DRMFileV23
    {
        private static List<DRMResInfoV23> m_ResourceInfo = new List<DRMResInfoV23>();
        ...
        {
            var m_Header = new DRMHeaderV23();
            var lpHeader = TDRMReader.ReadBytes(36);
            using (var THeaderReader = new MemoryStream(lpHeader))
            {
                ...
                m_Header.dwLocale = THeaderReader.ReadUInt64();
                THeaderReader.Dispose();
            }

            using (var TResInfoReader = new MemoryStream(TDRMReader.ReadBytes(m_Header.dwTotalSections * 24)))
            {
                for (Int32 i = 0; i < m_Header.dwTotalSections; i++)
                {
                    var TResInfo = new DRMResInfoV23
                    {
                        ...
                        dwLocale = TResInfoReader.ReadUInt64(),
                    };
            ...
            if (dwStringsDataSize != 0)
            {
                do
                {
                    UInt64 dwLocale = TDRMReader.ReadUInt64();
                    String m_LinkedResource = TDRMReader.ReadString();
                    Utils.iSetInfo("[DRM INFO]: Linked Resource: " + dwLocale.ToString("X16") + @"\" + m_LinkedResource);
                    //File.AppendAllText(@"LinkedResources.log", m_LinkedResource + Environment.NewLine);
                } while (TDRMReader.Position != dwLimitOffset);
            }

            using (var TResEntryReader = new MemoryStream(TDRMReader.ReadBytes(m_Header.dwTotalSections * 24)))
            {
                for (Int32 i = 0; i < m_Header.dwTotalSections; i++)
                {
                    var TResEntry = new DRMResEntry
                    {
                        wUnknown1 = TResEntryReader.ReadUInt16(),
                        wUnknown2 = TResEntryReader.ReadUInt16(),
                        dwUnknown1 = TResEntryReader.ReadUInt32(),
                        wTigerPart = TResEntryReader.ReadUInt16(),
                        wTigerID = TResEntryReader.ReadUInt16(),
                        dwOffset = TResEntryReader.ReadUInt32(),
                        dwSize = TResEntryReader.ReadInt32(),
                        dwHash = TResEntryReader.ReadUInt32(),
                    };
                ...
                if (File.Exists(m_TigerArhive))
                {
                    using (FileStream TTigerStream = File.OpenRead(m_TigerArhive))
                    {
                        if (TTigerStream.Length > 56)
                        {
                            ...
                            {
                                m_FileName = DRMUtils.iCheckDTPResourceV23(lpBuffer, m_FileName);
                            }
                        ...
                        {
                            Utils.iSetWarning("[SKIPPED]: " + m_FileName + " -> file " + m_TigerName + " is empty");
                        }
                    }
                }
                else
